...
pipeline {
    ...
    options {
        ...
        timeout(time: 4, unit: 'HOURS') // Полная выгрузка ERP длиннее инкремента
    }
        ...
        stage('Checkout Source Code') {
            steps {
                script {
                    ...
                    // git add его не берёт, пока строка есть в .gitignore.
            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'repo_user_pass', usernameVariable: 'STORAGE_USER', passwordVariable: 'STORAGE_PASS')]) {
                        def srcCfPath = "${utils.shortWorkspace()}\\src\\cf"
